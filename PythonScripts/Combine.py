...
# To use: python Combine -d ./images -o ./videos/new_video.mp4 -e jpg -t 25
# Use this to sort: for i in `ls *.pdf`; do convert -density 300 "$i" "$i".jpg; done
...
arg_parser.add_argument("-t", "--time", required=False, default='none',
...
if isnum(time):
    framerate = int(time)
    # int(len(images) / int(time))
    print("Adjusting framerate to " + str(time))
